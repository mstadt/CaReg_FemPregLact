...
if save_SS
    ...
    save(fname_save, 'SS', 'valsSS','par_malepreg', 'params', 'param_names')
...
if save_SS
    ...
    save(fname_save, 'SS', 'valsSS','par_malelact', 'params', 'param_names')
...
%% lower calcitriol synthesis -- preg
%par_malepreg(22) = pars_preg(22); % preg k_conv_min
%par_malepreg(21) = pars_preg(21); % preg delta_conv_max
...
SSpregfile = './SSbest/16-May-2023_calcium_mod_SS_sexORrep-preg_notes-PTHupdate2.mat';
SSpreg = load(SSpregfile).SS;
Vp_preg = pars_preg(18);
SS_IG(4) = SSpreg(4)/Vp_preg * Vp_malepreg;
...
sgtitle('ODE trajectory for preg male low calcitriol model')
SS_IG = y(end,:)';
...
fprintf('final steady states for low calcitriol preg male \n')
fprintf('           SS\n')
fprintf('PTHg:      %0.3f\n', SS(1))
fprintf('PTHp_con:  %0.3f\n', SS(2)/Vp_malepreg)
fprintf('Cap_con:   %0.3f\n', SS(3)/Vp_malepreg)
...
if save_SS
    notes = input('notes: ');
    fname_save = strcat('./SS/',date,'_calcium_mod_SS', '_sexORrep-malepreg_lowcalcitriol_notes-',notes,'.mat');
    save(fname_save, 'SS', 'valsSS','par_malepreg', 'params', 'param_names')

    fprintf('male preg low calcitriol results saved to: \n %s \n', fname_save);
end

clear params
%% lower calcitriol synthesis -- lact
% par_malelact(22) = pars_lact(22); % lact k_conv_min
% par_malelact(21) = pars_lact(21); % lact delta_conv_max
...
SSlactfile = './SSbest/16-May-2023_calcium_mod_SS_sexORrep-lact_notes-PTHupdate2.mat';
SSlact = load(SSpregfile).SS;
Vp_lact = pars_lact(18);
SS_IG(4) = SSpreg(4)/Vp_lact * Vp_malelact;

tspan = [0 5000];
options = odeset('RelTol', 1.0e-6, 'AbsTol', 1e-9);
fprintf('solving lact male low calcitriol model eqns \n')
IC = SS_IG;
...
sgtitle('ODE trajectory for lact male low calcitriol model')
SS_IG = y(end,:)';
...
fprintf('final steady states for low calcitriol lact male \n')
fprintf('           SS\n')
fprintf('PTHg:      %0.3f\n', SS(1))
fprintf('PTHp_con:  %0.3f\n', SS(2)/Vp_malelact)
fprintf('Cap_con:   %0.3f\n', SS(3)/Vp_malelact)
...
if save_SS
    notes = input('notes: ');
    fname_save = strcat('./SS/',date,'_calcium_mod_SS', '_sexORrep-malelact_lowcalcitriol_notes-',notes,'.mat');
    save(fname_save, 'SS', 'valsSS','par_malelact', 'params', 'param_names')

    fprintf('male lact low calcitriol results saved to: \n %s \n', fname_save);
end