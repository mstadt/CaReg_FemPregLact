...
male2female_sens = zeros(length(diffIDs), 7);  % col 1: PTH, col 2: Ca, col 3: calcitriol, col4: gut abs, col 5: bone 2 plas, col6: urine, col7: plas2bone
...
for ii = 1:length(diffIDs)
    ...
    male2female_sens(ii,:) = SSsens;
    male2female_frac(ii,:) = 100.0*(male2female_sens(ii,:) - male_base')./male_base';

    SSsens = compute_SS(ICfemale, pars_male, pars_female, parID);
    female2male_sens(ii,:) = SSsens;
    female2male_frac(ii,:) = 100.0*(female2male_sens(ii,:) - female_base')./female_base';
end
...
if saveres
    fname = strcat('./results_male2fem_fem2male/', date, ...
                '_male2femalesens_withflux', 'notes-', notes, '.mat');
...
%-----------------------------------------------------
function SS_vals = compute_SS(IC, pars1, pars2, IDchange)
    ...
    SS_vals = zeros(7,1);
    SS_vals(1:3) = SS(2:4)/Vp; % change to concentration
    
    % get fluxes
    vals = compute_vars(SS', pars2);
    SS_vals(4) = vals.Gut_absorption;
    SS_vals(5) = vals.FastPool_to_Plasma + vals.Bone_resorption;
    %SS_vals(6) = vals.Urine_excretion;
    SS_vals(7) = vals.Plasma_to_FastPool;
end